...
var editandoAjusteSalarialId = null; // id do ajuste salarial (CLT) sendo corrigido, se houver
var editandoAjusteHonorarioId = null; // id do ajuste de honorário (PJ) sendo corrigido, se houver
...
// ------------------------- Esqueci minha senha -------------------------
el("linkEsqueciSenha").addEventListener("click", function () {
  el("esqueciSenhaEmail").value = el("loginEmail").value.trim();
  el("esqueciSenhaError").hidden = true;
  el("esqueciSenhaOk").hidden = true;
  el("loginForm").hidden = true;
  el("esqueciSenhaForm").hidden = false;
  el("loginEyebrow").textContent = "Recuperar acesso";
  el("loginTitulo").textContent = "Esqueci minha senha";
  el("loginSub").textContent = "Sem problema — vamos te ajudar a criar uma senha nova.";
});
el("linkVoltarLogin").addEventListener("click", function () {
  el("esqueciSenhaForm").hidden = true;
  el("loginForm").hidden = false;
  el("loginError").hidden = true;
  el("loginEyebrow").textContent = "Acesso ao sistema";
  el("loginTitulo").textContent = "Entrar";
  el("loginSub").textContent = "Use o e-mail e a senha cadastrados para acessar sua conta.";
});
el("esqueciSenhaForm").addEventListener("submit", function (e) {
  e.preventDefault();
  var email = el("esqueciSenhaEmail").value.trim();
  var erroBox = el("esqueciSenhaError");
  var okBox = el("esqueciSenhaOk");
  var btn = el("btnEsqueciSenha");
  erroBox.hidden = true; okBox.hidden = true;
  btn.disabled = true; btn.textContent = "Enviando...";
  resetarSenhaPorEmail(email).then(function () {
    okBox.textContent = "Se esse e-mail estiver cadastrado, você vai receber um link para criar uma senha nova em alguns minutos. Não esqueça de checar a caixa de spam/lixo eletrônico.";
    okBox.hidden = false;
    el("esqueciSenhaForm").reset();
  }).catch(function (err) {
    // Não revela se o e-mail existe ou não (evita que alguém descubra
    // quais e-mails têm conta só tentando resetar a senha deles).
    var msg = "Se esse e-mail estiver cadastrado, você vai receber um link para criar uma senha nova em alguns minutos. Não esqueça de checar a caixa de spam/lixo eletrônico.";
    if (err && err.code === "auth/invalid-email") msg = "E-mail inválido.";
    if (err && err.code === "auth/too-many-requests") msg = "Muitas tentativas. Aguarde um pouco e tente de novo.";
    if (err && (err.code === "auth/user-not-found")) { okBox.textContent = "Se esse e-mail estiver cadastrado, você vai receber um link para criar uma senha nova em alguns minutos. Não esqueça de checar a caixa de spam/lixo eletrônico."; okBox.hidden = false; }
    else { erroBox.textContent = msg; erroBox.hidden = false; }
  }).finally(function () {
...
function renderAjustesSalariais() {
  ...
  body.innerHTML = ajustesSalariais.map(function (a) {
    return '<div class="timeline-item">' +
      ...
      '<div class="timeline-corpo"><div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;">' +
      '<strong>' + (a.cargoNovo ? 'Promoção / ajuste' : 'Ajuste salarial') + '</strong>' +
      '<span class="row-actions">' +
        '<button type="button" class="btn-icon" data-action="editar-ajuste-salarial" data-id="' + a.id + '" title="Corrigir este ajuste"><svg viewBox="0 0 24 24" fill="none"><path d="M4 20h4l10.5-10.5a2 2 0 0 0 0-2.8l-1.2-1.2a2 2 0 0 0-2.8 0L4 16v4z" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/></svg></button>' +
        '<button type="button" class="btn-icon danger" data-action="excluir-ajuste-salarial" data-id="' + a.id + '" title="Excluir este ajuste"><svg viewBox="0 0 24 24" fill="none"><path d="M5 7h14M9 7V5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2m-9 0 1 13a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1l1-13" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg></button>' +
      '</span></div>' +
      ...
      '<small style="color:var(--ink-faint);">registrado por ' + (a.criadoPorNome || '—') + (a.atualizadoPorNome ? ' · corrigido por ' + a.atualizadoPorNome : '') + '</small>' +
      '</div></div>';
  }).join("");
}
el("ajustesSalariaisBody").addEventListener("click", function (e) {
  var btn = e.target.closest("button[data-action]");
  if (!btn) return;
  var id = btn.dataset.id;
  if (btn.dataset.action === "editar-ajuste-salarial") editarAjusteSalarial(id);
  if (btn.dataset.action === "excluir-ajuste-salarial") excluirAjusteSalarialUI(id);
});
function editarAjusteSalarial(id) {
  var a = ajustesSalariais.filter(function (x) { return x.id === id; })[0];
  if (!a) return;
  editandoAjusteSalarialId = id;
  el("ajusteSalarialData").value = a.dataAjuste || "";
  el("ajusteSalarialValor").value = a.valorNovo ? numeroParaMoeda(a.valorNovo) : "";
  el("ajusteSalarialCargo").value = a.cargoNovo || "";
  el("ajusteSalarialMotivo").value = a.motivo || "";
  el("ajusteSalarialFormError").hidden = true;
  el("ajusteSalarialCancelEdit").hidden = false;
  el("ajusteSalarialSubmitBtn").textContent = "Salvar correção";
  el("ajusteSalarialForm").scrollIntoView({ behavior: "smooth", block: "center" });
}
function cancelarEdicaoAjusteSalarial() {
  editandoAjusteSalarialId = null;
  el("ajusteSalarialForm").reset();
  el("ajusteSalarialFormError").hidden = true;
  el("ajusteSalarialCancelEdit").hidden = true;
  el("ajusteSalarialSubmitBtn").textContent = "Registrar ajuste";
}
el("ajusteSalarialCancelEdit").addEventListener("click", cancelarEdicaoAjusteSalarial);
function excluirAjusteSalarialUI(id) {
  abrirConfirmacao("Excluir este ajuste?", "Essa ação não pode ser desfeita. O cadastro do colaborador não é alterado automaticamente — se esse era o ajuste mais recente, corrija o cargo/salário atual manualmente no formulário acima, se precisar.", function () {
    setSync("syncing");
    excluirAjusteSalarial(id).then(function () {
      setSync("ok"); showToast("Ajuste excluído.");
      if (editandoAjusteSalarialId === id) cancelarEdicaoAjusteSalarial();
      return carregarAjustesSalariais(editandoEmpId);
    }).catch(function (err) { console.error(err); setSync("error"); showToast("Não foi possível excluir: " + (err && err.message || "")); });
  });
}
el("ajusteSalarialForm").addEventListener("submit", function (e) {
  ...
  var idEmEdicao = editandoAjusteSalarialId;
  var ajusteOriginal = idEmEdicao ? ajustesSalariais.filter(function (x) { return x.id === idEmEdicao; })[0] : null;

  var registro = {
    colaboradorId: editandoEmpId,
    colaboradorNome: colaborador.nome,
    dataAjuste: dataAjuste,
    valorAnterior: valorNovo ? (ajusteOriginal ? (ajusteOriginal.valorAnterior || 0) : (colaborador.salarioBase || 0)) : null,
    valorNovo: valorNovo,
    cargoAnterior: cargoNovo ? (ajusteOriginal ? (ajusteOriginal.cargoAnterior || null) : (colaborador.cargo || null)) : null,
  ...
  setSync("syncing");
  var promessaSalvar = idEmEdicao ? atualizarAjusteSalarial(idEmEdicao, registro) : salvarAjusteSalarial(registro);
  promessaSalvar
    .then(function () { return salvarColaborador(dadosColaborador); })
    .then(function () {
      setSync("ok"); showToast(idEmEdicao ? "Ajuste corrigido." : (cargoNovo ? "Promoção/ajuste registrado." : "Ajuste salarial registrado."));
      cancelarEdicaoAjusteSalarial();
...
el("gozoFuncionario").addEventListener("change", function () {
  ...
  // Mostra todo período calculado a partir da admissão — inclusive o que
  // ainda está em andamento (não completou os 12 meses ainda), marcado
  // como tal. Só escondemos período já 100% gozado (saldo zerado) E
  // completo, pra não sujar a lista com histórico já quitado.
  var todosPeriodos = periodosAquisitivos(c, new Date().toISOString().slice(0, 10), gozosDoColab);
  var periodos = todosPeriodos.filter(function (p) { return !p.completo || p.saldo > 0; });
  if (periodos.length === 0) { select.innerHTML = '<option value="" disabled selected>Nenhum período com saldo disponível</option>'; return; }
  select.innerHTML = '<option value="" disabled selected>Selecione</option>';
  periodos.forEach(function (p) {
    var opt = document.createElement("option");
    opt.value = p.inicio;
    opt.textContent = formatDateBR(p.inicio) + ' – ' + formatDateBR(p.fim) +
      (p.completo ? ' (saldo: ' + p.saldo + ' dias)' : ' (em andamento — ' + p.mesesDecorridos + '/12 meses, ainda sem direito adquirido)');
    select.appendChild(opt);
  });
});
function atualizarPreviewDiasGozo() {
  var inicio = el("gozoInicio").value, fim = el("gozoFim").value;
  var preview = el("gozoDiasPreview");
  if (!inicio || !fim || fim < inicio) { preview.hidden = true; return; }
  var dias = diffDias(inicio, fim) + 1;
  preview.hidden = false;
  preview.textContent = dias + " dia" + (dias === 1 ? "" : "s") + " a serem tirados.";
}
el("gozoInicio").addEventListener("change", atualizarPreviewDiasGozo);
el("gozoFim").addEventListener("change", atualizarPreviewDiasGozo);
el("gozoForm").addEventListener("submit", function (e) {
  ...
  var motivo = el("gozoMotivo").value.trim();
  var erro = null;
  ...
  setSync("syncing");
  salvarFeriasGozo({ colaboradorId: colabId, periodoAquisitivoInicio: periodoInicio, dataInicio: dataInicio, dataFim: dataFim, dias: dias, motivo: motivo }).then(function () {
    setSync("ok"); showToast("Férias lançadas."); el("gozoForm").reset(); el("gozoDiasPreview").hidden = true; return carregarTudo(false);
  }).catch(function (err) { console.error(err); setSync("error"); showToast("Não foi possível salvar: " + (err && err.message || "")); });
});
function renderGozos() {
  ...
  feriasGozos.slice().sort(function (a, b) { return a.dataInicio < b.dataInicio ? 1 : -1; }).forEach(function (g) {
    ...
    tr.innerHTML = '<td>' + (c ? c.nome : '(colaborador removido)') + '</td><td>' + formatDateBR(g.periodoAquisitivoInicio) + '</td><td>' + formatDateBR(g.dataInicio) + ' – ' + formatDateBR(g.dataFim) + '</td><td>' + g.dias + '</td><td>' + (g.motivo || '—') + '</td><td><small style="color:var(--ink-faint);">' + (g.atualizadoPorNome || g.criadoPorNome || '—') + '</small></td><td>' + acoes + '</td>';
    body.appendChild(tr);
  });
}
...
function atualizarPreviewDiasPjPeriodo() {
  var inicio = el("pjPeriodoInicio").value, fim = el("pjPeriodoFim").value;
  var preview = el("pjPeriodoDiasPreview");
  if (!inicio || !fim || fim < inicio) { preview.hidden = true; return; }
  var dias = diffDias(inicio, fim) + 1;
  preview.hidden = false;
  preview.textContent = dias + " dia" + (dias === 1 ? "" : "s") + " a serem tirados.";
}
el("pjPeriodoInicio").addEventListener("change", atualizarPreviewDiasPjPeriodo);
el("pjPeriodoFim").addEventListener("change", atualizarPreviewDiasPjPeriodo);
el("pjPeriodoForm").addEventListener("submit", function (e) {
  ...
  salvarPjPeriodo({ prestadorId: prestadorId, dataInicio: inicio, dataFim: fim, dias: dias, observacao: el("pjPeriodoObs").value.trim() })
    .then(function () { showToast("Período lançado."); el("pjPeriodoForm").reset(); el("pjPeriodoDiasPreview").hidden = true; return carregarPJ(); })
...
function renderAjustesHonorarioPJ() {
  ...
  body.innerHTML = ajustesHonorarioPJ.map(function (a) {
    return '<div class="timeline-item">' +
      '<div class="timeline-data">' + formatDateBR(a.dataAjuste) + '</div>' +
      '<div class="timeline-corpo"><div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;">' +
      '<strong>' + (a.cargoNovo ? 'Promoção / ajuste' : 'Ajuste de honorário') + '</strong>' +
      '<span class="row-actions">' +
        '<button type="button" class="btn-icon" data-action="editar-ajuste-honorario" data-id="' + a.id + '" title="Corrigir este ajuste"><svg viewBox="0 0 24 24" fill="none"><path d="M4 20h4l10.5-10.5a2 2 0 0 0 0-2.8l-1.2-1.2a2 2 0 0 0-2.8 0L4 16v4z" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/></svg></button>' +
        '<button type="button" class="btn-icon danger" data-action="excluir-ajuste-honorario" data-id="' + a.id + '" title="Excluir este ajuste"><svg viewBox="0 0 24 24" fill="none"><path d="M5 7h14M9 7V5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2m-9 0 1 13a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1l1-13" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg></button>' +
      '</span></div>' +
...
el("ajustesHonorarioBody").addEventListener("click", function (e) {
  var btn = e.target.closest("button[data-action]");
  if (!btn) return;
  var id = btn.dataset.id;
  if (btn.dataset.action === "editar-ajuste-honorario") editarAjusteHonorario(id);
  if (btn.dataset.action === "excluir-ajuste-honorario") excluirAjusteHonorarioUI(id);
});
function editarAjusteHonorario(id) {
  var a = ajustesHonorarioPJ.filter(function (x) { return x.id === id; })[0];
  if (!a) return;
  editandoAjusteHonorarioId = id;
  el("ajusteHonorarioData").value = a.dataAjuste || "";
  el("ajusteHonorarioValor").value = a.valorNovo ? numeroParaMoeda(a.valorNovo) : "";
  el("ajusteHonorarioCargo").value = a.cargoNovo || "";
  el("ajusteHonorarioMotivo").value = a.motivo || "";
  el("ajusteHonorarioFormError").hidden = true;
  el("ajusteHonorarioCancelEdit").hidden = false;
  el("ajusteHonorarioSubmitBtn").textContent = "Salvar correção";
  el("ajusteHonorarioForm").scrollIntoView({ behavior: "smooth", block: "center" });
}
function cancelarEdicaoAjusteHonorario() {
  editandoAjusteHonorarioId = null;
  el("ajusteHonorarioForm").reset();
  el("ajusteHonorarioFormError").hidden = true;
  el("ajusteHonorarioCancelEdit").hidden = true;
  el("ajusteHonorarioSubmitBtn").textContent = "Registrar ajuste";
}
el("ajusteHonorarioCancelEdit").addEventListener("click", cancelarEdicaoAjusteHonorario);
function excluirAjusteHonorarioUI(id) {
  abrirConfirmacao("Excluir este ajuste?", "Essa ação não pode ser desfeita. O cadastro do prestador não é alterado automaticamente — se esse era o ajuste mais recente, corrija o cargo/honorário atual manualmente no formulário acima, se precisar.", function () {
    excluirAjusteHonorarioPJ(id).then(function () {
      showToast("Ajuste excluído.");
      if (editandoAjusteHonorarioId === id) cancelarEdicaoAjusteHonorario();
      return carregarAjustesHonorarioPJ(editandoPjId);
    }).catch(function (err) { console.error(err); showToast("Não foi possível excluir: " + (err && err.message || "")); });
  });
}
el("ajusteHonorarioForm").addEventListener("submit", function (e) {
  ...
  var idEmEdicao = editandoAjusteHonorarioId;
  var ajusteOriginal = idEmEdicao ? ajustesHonorarioPJ.filter(function (x) { return x.id === idEmEdicao; })[0] : null;

  var registro = {
    prestadorId: editandoPjId,
    prestadorNome: pj.nomeRazaoSocial,
    dataAjuste: dataAjuste,
    valorAnterior: valorNovo ? (ajusteOriginal ? (ajusteOriginal.valorAnterior || 0) : (pj.valorHonorarioMensal || 0)) : null,
    valorNovo: valorNovo,
    cargoAnterior: cargoNovo ? (ajusteOriginal ? (ajusteOriginal.cargoAnterior || null) : (pj.cargoFuncao || null)) : null,
    cargoNovo: cargoNovo || null,
    motivo: motivo,
  };
  ...
  var promessaSalvar = idEmEdicao ? atualizarAjusteHonorarioPJ(idEmEdicao, registro) : salvarAjusteHonorarioPJ(registro);
  promessaSalvar
    .then(function () { return salvarPrestadorPJ(dadosPj); })
    .then(function () {
      showToast(idEmEdicao ? "Ajuste corrigido." : (cargoNovo ? "Promoção/ajuste registrado." : "Ajuste de honorário registrado."));
      cancelarEdicaoAjusteHonorario();
  ...
  promessa.then(function (results) {
    ...
    if (tipo === "clt") {
      ajustes.forEach(function (a) {
        eventos.push({ data: a.dataAjuste, ordem: 1, titulo: a.cargoNovo ? "Promoção / ajuste" : "Ajuste salarial", cargo: valorNaData(pontosCargo, a.dataAjuste), valorLabel: rotuloValor, valor: valorNaData(pontosValor, a.dataAjuste), detalhe: a.motivo || "", autor: a.criadoPorNome });
      });
      ...
      ajustes.forEach(function (a) {
        eventos.push({ data: a.dataAjuste, ordem: 1, titulo: a.cargoNovo ? "Promoção / ajuste" : "Ajuste de honorário", cargo: valorNaData(pontosCargo, a.dataAjuste), valorLabel: rotuloValor, valor: valorNaData(pontosValor, a.dataAjuste), detalhe: a.motivo || "", autor: a.criadoPorNome });
      });
